...
class Task(models.Model):
    ...
    contributer_ids = fields.Many2many('res.users', string="Contributers")
    ...
    def send_subtask_email(self, subtask_name, subtask_state, subtask_reviewer_id, subtask_user_id, old_name=None):
        for r in self:
            ...
            if user == self.env.user and reviewer == self.env.user:
                ...
                body = '<p>' + 'Dear ' + escape(reviewer.name) + ',<br></br> <br></br><em style="color:#999">I updated checklist item assigned to me:</em> <br><strong>' + state + '</strong>: ' + escape(subtask_name)
